...
def get_status():
    ...
    # Identity fix to bypass Reddit bot-blocks
    ...
    try:
        ...
        for entry in feed.entries[:20]:
            ...
            # Target official maintenance announcements
            if "the division 2" in title.lower() and "maintenance" in title.lower():
                date_match = re.search(r'([A-Z][a-z]+ \d{1,2}, \d{4})', title)
                if date_match:
                    date_str = date_match.group(1)
                    # Convert to datetime (Assuming standard 08:30 UTC start)
                    start_time = datetime.strptime(date_str, "%B %d, %Y").replace(hour=8, minute=30)
                    
                    # Extract downtime duration (e.g., "5 hours")
                    duration_match = re.search(r'estimated downtime is (\d+) hours', content.lower())
        ...
        return {"title": "All Systems Nominal", "url": "", "utc_start": "", "utc_end": ""}
    except:
